        ...
        for morphed_image_name in frs_score_dict_list[0]:
            for subject_id in frs_score_dict_list[0][morphed_image_name]:
                ...
                if len(frs_score_dict_list[0][morphed_image_name][subject_id]) != len(
                    ...
                    print(
                        "Error: not all files contain the same number of scores for the morphed image {0} and the subject {1}".format(
                            morphed_image_name, subject_id
                        )
                    )
                    exit(1)
...
def main():
    ...
    print("Loading scores in progress...")
    frs_score_dict_list = []
    frs_thr_list = []
    ...
    print("Loading scores finished.")

    print("Score consistency check in progress...")
    check_score_dict_consistency(frs_score_dict_list)
    print("Score consistency check finished.")
    
    print("MAP computation in progress...")
    map, map_count = MAP.compute_map(
        frs_score_dict_list, frs_thr_list, frs_is_similarity_list
    )
    print("MAP computation finished.")
    print("===============")
    print("MAP:")
    print_map(map)
    print("===============")
    ...
    save_map_to_text_file(
        "{}/MAP.txt".format(output_folder_path), map, len(frs_score_dict_list[0].keys())
    )
    save_map_to_text_file(
        "{}/MAPCount.txt".format(output_folder_path),
        map_count,
